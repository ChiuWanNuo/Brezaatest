...
class FirstTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let urlStr = "https://api.adorable.io/avatars/285/\(userinfo.username)@adorable.png"
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let controller = segue.destination as? SecondTableViewController, let row = self.tableView.indexPathForSelectedRow?.row {
            let usersdata = Users[row]
            controller.Users = usersdata
        }
    }
